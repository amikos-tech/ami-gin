...
| T18-05 | Tampering | mitigate | CLOSED | `failure_modes_test.go:102` provides the hard-ingest behavior matrix; `ingest_error_guard_test.go:11` executes the guard and `ingest_error_guard_test.go:143` defines the auto-discovered hard-ingest surface set. |
| T18-06 | Denial of Service | mitigate | CLOSED | `getOrCreateStagedPath` enforces `MaxStagedPaths` against every staged path, including companion representations; `TestMaxStagedPaths` covers container-derived and deep-array rejection. `ingest_error_guard_test.go` keeps this hard-ingest boundary in the automated error-surface audit. |
...
| T18-11 | Repudiation | mitigate | CLOSED | `experimentIngestFailureGroups` and `experimentIngestLayerRank` pin parser/transformer/numeric/schema/resource/unknown ordering; `TestExperimentIngestFailureGroupsDeterministic` and the JSON report tests assert stable order and resource grouping. |
| T18-12 | Information Disclosure | mitigate | CLOSED | `IngestError.Value` documents verbatim, caller-owned document-data values and empty resource values; `Cause` carries resource diagnostics. The Unreleased changelog repeats the release-facing contract. |
