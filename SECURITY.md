...
| T18-06 | Denial of Service | mitigate | CLOSED | `ingest_error_guard_test.go:3`, `ingest_error_guard_test.go:10`, and `ingest_error_guard_test.go:21` implement the guard with stdlib AST parsing scoped to seven named functions; `ingest_error_guard_test.go:108` documents the intentional scope. |
...
| T18-09 | Tampering | mitigate | CLOSED | `cmd/gin-index/experiment.go:487` stores failed-line `input_index` separately from accepted docs; `cmd/gin-index/experiment_test.go:689` checks parser sample line/input-index; `cmd/gin-index/experiment_test.go:716` and `cmd/gin-index/experiment_test.go:722` assert 90 accepted docs collapse to 9 row groups. |
...
| T18-11 | Repudiation | mitigate | CLOSED | `cmd/gin-index/experiment.go:505` sorts failure groups deterministically and `cmd/gin-index/experiment.go:535` pins parser/transformer/numeric/schema ordering; `cmd/gin-index/experiment_test.go:239`, `cmd/gin-index/experiment_test.go:674`, and `cmd/gin-index/experiment_test.go:737` assert stable order. |
| T18-12 | Information Disclosure | mitigate | CLOSED | `ingest_error.go:25` documents that `Value` is verbatim and caller-owned for redaction/output policy; `CHANGELOG.md:5` repeats the same release-facing warning. |
