...
- **Encode the vacuity check as a `t.Fatalf` guard, not a mental note.** Spike 003's Q1 returned suspiciously perfect ±0.0% deltas — exactly what a silently-disengaged `WithParser` would produce. Rather than trust it, the next test asserted "if the SIMD arm is within 2% of stdlib on *all* fixtures, fail" — so the harness itself refuses to report a meaningless result. Guards outlive attention.
- **Before trusting a performance delta, prove both arms did the same work.** Spike 003 asserted byte-identical encoded output across the stdlib and SIMD arms; without that, "X is 1.5× slower" could just mean X quietly did less.
- **Repeat a surprising measurement before explaining it.** Spike 003's Q2 showed a 3× fresh-vs-shared split across two of four fixtures; 5 repeats × 4 fixtures collapsed it to ±2.5% — it was machine noise. Report `testing.Benchmark`'s chosen `N` alongside `ns/op` so small-N noise is visible rather than inferred.
- **Drive `testing.Benchmark()` from an ordinary `Test` function** when the question is about *ordering* or *lifecycle*. It gives precise control over what runs in which order inside one process, and lets results print as a comparison table instead of scattered `-bench` output that must be diffed by eye.
...
- **Go's `B/op` and `allocs/op` count Go-heap allocations only.** pure-simdjson's parser buffers live in native memory reached through purego, so Go's allocator never sees them. Upstream has native telemetry (`NativeAllocStatsReset`/`NativeAllocStatsSnapshot`, `benchmark_native_alloc_test.go`) but it is package-internal — the exported `Parser` surface is only `Parse`/`Close`. From outside the module, process RSS (`ps -o rss=`) is the only native-growth signal available.
- Package-internal helpers in the repo's own `_test.go` files (e.g. `phase20BuildBenchmarkIndex` in `benchmark_test.go`) **cannot be imported** by an isolated spike module. Mirror them against the public API and cite the source line, rather than abandoning module isolation.
