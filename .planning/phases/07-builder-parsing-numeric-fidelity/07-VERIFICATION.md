...
| 5 | BUILD-05 benchmark evidence is reproducible on the current tree with an in-repo legacy control and explicit parser-mode labeling. | ✓ VERIFIED | `benchmark_test.go:206-229` defines the fixed doc-count/shape matrix; `benchmark_test.go:564-588` keeps the benchmark-only legacy control path (`benchmarkAddDocumentLegacy`); `benchmark_test.go:972-1065` defines `BenchmarkAddDocumentPhase07`, `BenchmarkBuildPhase07`, and `BenchmarkFinalizePhase07`; the benchmark smoke command passed on HEAD and emitted both parser modes across all shapes/doc counts. |
...
| `benchmark_test.go` | Legacy-vs-explicit Phase 07 benchmark families with explicit parser/docs/shape labels | ✓ VERIFIED | `benchmark_test.go:206-229,564-588,972-1065` defines the matrix, legacy control path, and benchmark families. |
...
| `BUILD-05` | `07-02-PLAN.md` | Benchmarks capture ingest/build/finalize latency and allocation deltas for the parser redesign | ✓ SATISFIED | `benchmark_test.go:206-229,564-588,972-1065` provides the fixed benchmark matrix and legacy control; the benchmark smoke command passed on HEAD with both parser modes across all four shapes and three doc counts. |
