...
| **Config file** | `go.mod` |
| **Quick run command** | `go test ./... -run 'Test(IngestErrorWrappingContract|HardIngestFailuresReturnIngestError|ParserContractErrorsRemainNonIngestError|SoftFailureModesDoNotReturnIngestError|HardIngestFunctionsDoNotReturnPlainErrors|RunExperimentOnErrorContinueIngestFailuresText|RunExperimentOnErrorContinueIngestFailuresJSON|RunExperimentHundredDocsKnownIngestFailuresJSON)$' -count=1` |
...
| 18-03-01 | 03 | 2 | IERR-03 | T18-04 | CLI groups returned `IngestError`s by layer with counts and at most 3 samples per layer in JSON. | integration | `go test ./cmd/gin-index -run 'Test(RunExperimentOnErrorContinueIngestFailuresJSON|RunExperimentHundredDocsKnownIngestFailuresJSON)$' -count=1` | yes | green |
| 18-03-02 | 03 | 2 | IERR-03 | T18-04 | CLI text mode reports layer counts and structured samples while accepted docs remain densely packed. | integration | `go test ./cmd/gin-index -run 'Test(RunExperimentOnErrorContinueIngestFailuresText|RunExperimentHundredDocsKnownIngestFailuresJSON)$' -count=1` | yes | green |
...
- Re-ran the Phase 18 focused contract, CLI, full-suite, and lint commands against the current tree.
- Confirmed all requirement rows remain COVERED by executable automated checks.
- Refreshed stale command text for the CLI coverage rows so the validation artifact now references the current test names.

Audit rerun evidence:

- `go test ./... -run 'Test(IngestErrorWrappingContract|HardIngestFailuresReturnIngestError|ParserContractErrorsRemainNonIngestError|SoftFailureModesDoNotReturnIngestError|HardIngestFunctionsDoNotReturnPlainErrors)$' -count=1` - PASS
- `go test ./cmd/gin-index -run 'TestRunExperiment(OnErrorContinueIngestFailuresJSON|OnErrorContinueIngestFailuresText|HundredDocsKnownIngestFailuresJSON)$' -count=1` - PASS
- `go test ./...` - PASS
- `make lint` - PASS
